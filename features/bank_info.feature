Feature: API endpoint to send user's bank details
  ...
  Scenario Outline: A user wants to create a record with "<case>"
    ...
    Examples: bank data
      ...
      | Missing payment_method                             | null           | US                | John Smith   | 123                   | ICBCUSBJ   | 11122233A  | ""      | 400         | {"error": "'payment_method' field required, the value should be either 'LOCAL' or 'SWIFT'"} |
      | Missing bank_country_code                          | SWIFT          | null              | John Smith   | 123                   | ICBCUSBJ   | 11122233A  | ""      | 400         | {"error": "'bank_country_code' is required, and should be one of 'US', 'AU', or 'CN'"} |
      | Missing account_name                               | SWIFT          | US                | null         | 123                   | ICBCUSBJ   | 11122233A  | ""      | 400         | {"error": "'account_name' is required"} |
      | Missing account_number                             | SWIFT          | US                | John Smith   | null                  | ICBCUSBJ   | 11122233A  | ""      | 400         | {"error": "'account_number' is required"} |
      | swift_code missing when payment method is SWIFT    | SWIFT          | US                | John Smith   | 123                   | null       | 11122233A  | ""      | 400         | {"error": "'swift_code' is required when payment method is 'SWIFT'"} |
      ...
      | aab missing for US                                 | SWIFT          | US                | John Smith   | 123                   | ICBCUSBJ   | null       | ""      | 400         | {"error": "'aba' is required when bank country code is 'US'"} |
